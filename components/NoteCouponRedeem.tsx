...
type NoteCouponRedeemProps = {
    classId: string;
    subjectId: string;
    onSuccess: () => void;
};

export default function NoteCouponRedeem({ classId, subjectId, onSuccess }: NoteCouponRedeemProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/user/redeem-note-coupon", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ classId, subjectId, code: trimmed }),
            });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                ...
                <input
                    ...
                    placeholder="Enter your note coupon code"
                ...
                />
                {!subjectId && (
                    <p className="text-xs text-slate-500 mt-1">Select a subject first to redeem note coupon</p>
                )}
            </div>
            ...
            <button
                type="submit"
                disabled={submitting || !code.trim() || !subjectId}
            ...
            >
                {submitting ? "Redeeming..." : "Redeem Note Coupon"}
            </button>
        </form>
    );
}
